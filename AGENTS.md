...
- `gitops` – Review-first publishing: clones the `vex.git.repo` into `<repo.cache_dir>/gitops`, rebuilds a `vexviper/<product-slug>` branch from the base on every run, commits the document, force-pushes and opens/updates a GitHub PR via `net/http`. Uses the `git` CLI through an injectable `Runner`; https credentials travel via `GIT_CONFIG_*` env (never argv) and are redacted from errors. Tests use a local bare repo (`git init --bare`) + `httptest` for the PR API.
- `evidence` – Deterministic evidence (its `Item.Kind`/`Strong` set is what `assesscache.Fingerprint` hashes): version comparison, dependency depth, govulncheck (`-json`, multi-module, `go run …@latest` fallback when the installed binary is too old, `findGoBin` when `go` is not on PATH), symbol grep, OSV details. govulncheck evidence is attached **only to `pkg:golang/` PURLs**; other ecosystems (npm, pypi, cargo, gem, composer, maven, nuget, pub) get `ecosystem.go` evidence: manifest depth → `direct_dependency`/`transitive`/`dev_dependency`/`manifest_not_found` (dropped when BOMHort's dependency graph already set `DirectKnown`), `lockgraph.go` lockfile graph (13 formats) → `dependency_path` + **Strong** `dev_dependency` when the package manager resolved the package outside the runtime closure, source import scan → `package_imported`/`import_not_found` (**Strong** only for npm/cargo/composer/pub when direct + sole consumer per lockfile graph + ≥1 source scanned), `rust.go` RustSec `affects.functions` lexical check → `symbol_referenced`/`symbol_not_referenced` (**Strong** when sole consumer), plus `no_reachability_analysis`. The strong rules are documented in README "Evidence per ecosystem" and INTEGRATION.md "Strong evidence"; never loosen them without updating both.
...
- GitOps publishing (`internal/gitops`) pushes only to bot-owned `vex.git.branch_prefix` branches with `-f`; direct commits to the base branch are a non-forced push and require `branch_prefix: ""` explicitly. The token is read from `vex.git.token_env` (default `GITHUB_TOKEN`), passed to git via `GIT_CONFIG_*` env — never in argv or the remote URL — and redacted from errors/logs.
